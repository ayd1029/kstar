<template>
  <q-page class="page-default">
    ...
    <div class="row q-pl-md row justify-center q-pt-sm q-pb-sm">
      <div class="col-12">
        {{ $t('menu_my_nft_project_list_description') }}
      </div>
    </div>
          ...
          <q-item clickable @click="goDetail(item.seq)">
            ...
            <q-item-section>
              <div class="row">
                ...
                <q-item-label class="col-12">{{ $t('project_seq') }} [ {{ item.seq }} ]</q-item-label>
...
export default defineComponent({
  ...
  methods: {
    ...
    goDetail(seq) {
      // 상세 화면으로 이동
      this.$router.push({ path: '/nft/nftProjectDetail', query: { s: seq }})
      // this.$refs.refTokenDetailModal.tokenSeq = seq
      // this.$refs.refTokenDetailModal.show()
    },
    refresher (done) {
    ...
    selectListMax() {
      ...
      this.$axios.get('/api/nft/selectMyNftProjectListLastPageNum',
    ...
    selectList(idx, done) {
      ...
      this.$axios.get('/api/nft/selectMyNftProjectList',
    ...
    goRegister() {
      // NFT 프로젝트 등록 화면으로 이동
      this.$router.push('/nft/registerNftProject')
    },
