<template>
  <q-page class="page-default">
    ...
    <div class="row justify justify-between q-pb-sm q-pl-md">
      ...
      <div class="row q-pt-sm q-pb-sm">
        <q-btn @click="exportTable" color="primary" size="sm" class="q-mr-sm" style="width: 125px; height: 30px;" no-caps outline>
    ...
    <q-pull-to-refresh @refresh="refresher">
      <q-infinite-scroll @load="loadMore" :offset="0" ref="infiniteScroll">
        ...
        <div v-for="item in applyResultList" :key="item.seq" @click="showDetail(item.seq)" style="cursor: pointer;z-index: 1;">
          <q-separator />
          <q-item clickable>
            <q-item-section>
              <div class="">
  ...
  <WhitelistApplyResultDetailModal ref="refWhitelistApplyResultDetailModal" />
</template>
...
export default defineComponent({
  ...
  methods: {
    ...
    showDetail(seq_whitelist_apply) {
      ...
      this.$refs.refWhitelistApplyResultDetailModal.seq = seq_whitelist_apply
      this.$refs.refWhitelistApplyResultDetailModal.seq_whitelist = this.seq
      this.$refs.refWhitelistApplyResultDetailModal.select_type = this.select_type // 타입
      this.$refs.refWhitelistApplyResultDetailModal.show()
    },
    refresher (done) {
    ...
    selectListMax() {
      ...
      this.$axios.get('/api/whitelist/selectWhitelistApplyResultListLastPageNum',
    ...
    selectList(idx, done) {
      ...
      this.$axios.get('/api/whitelist/selectWhitelistApplyResultList',
    ...
    async exportTable() {
      ...
      try {
        ...
        const result = await this.$axios.get('/api/whitelist/selectWhitelistApplyResultListForDownload', {params: {uid: this.getUid, seq_whitelist: this.seq}})
        const applyResultList = result.data
        console.log(applyResultList)

        // 2. 화이트리스트 입력항목 조회
        const fieldList = await this.selectWhitelistFieldList(this.seq)
        ...
        // 3.2 데이터 설정
        if (applyResultList) {
          for (let i = 0; i < applyResultList.length; i++) {
            const dataObj = applyResultList[i]
    ...
    // 입력항목 리스트 조회
    async selectWhitelistFieldList(seq) {
      const param = {
        uid: this.getUid,
        seqWhitelist: seq,
      }
