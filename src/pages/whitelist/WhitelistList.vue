  ...
  <q-page class="page-default">
    ...
    <q-pull-to-refresh @refresh="refresher">
      <q-infinite-scroll @load="loadMore" :offset="0" ref="infiniteScroll">
        ...
        <div v-for="item in whitelistList" :key="item.seq" @click="goDetail(item.seq)" style="cursor: pointer; z-index: 1;">
          <q-separator />
          <q-item clickable>
...
export default defineComponent({
  ...
  methods: {
    ...
    selectListMax() {
      ...
      this.$axios.get('/api/whitelist/selectWhitelistListLastPageNum',
    ...
    selectList(idx, done) {
      ...
      this.$axios.get('/api/whitelist/selectWhitelistList',
        {params: {uid: this.getUid, pageNum: idx, pageSize: this.pageSize, keyword: this.keyword}})
        .then((result) => {
          // console.log(JSON.stringify(result.data))
          // console.log(result.data)
          if (idx === 1) { // 첫번째 load인 경우
            this.whitelistList = [] // 리스트 초기화
          }
          this.whitelistList = this.whitelistList.concat(result.data)

          // 데이터 없음 표시 설정
          if (!this.whitelistList || this.whitelistList.length < 1) {
