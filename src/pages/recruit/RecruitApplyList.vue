<template>
  <q-page class="page-default">
    ...
    <q-pull-to-refresh @refresh="refresher">
      <q-infinite-scroll @load="loadMore" :offset="0" ref="infiniteScroll">
        ...
        <div v-for="(item) in applyList" :key="item.seq" @click="showDetail(item.seq)" style="cursor: pointer;z-index: 1;">
          <q-separator />
          <q-item clickable>
            <q-item-section>
              <div class="">
                <q-item-label class="col-12 text-left text-weight-bold">{{ item.seq }}. {{ item.email }}</q-item-label>
                <q-item-label class="col-12 text-right">
                  {{ $t('recruit_apply_date') }} : {{ qDate.formatDate(item.reg_time, 'YYYY-MM-DD HH:mm') }}
                </q-item-label>
              </div>
            </q-item-section>
          </q-item>
  ...
  <RecruitApplyDetailModal ref="refRecruitApplyDetailModal" />
</template>
...
export default defineComponent({
  ...
  methods: {
    ...
    showDetail(seq_recruit_apply) {
      // 상세 화면으로 이동
      // this.$router.push({ path: '/token/detail', query: { seq: seq }})
      this.$refs.refRecruitApplyDetailModal.seq = seq_recruit_apply
      this.$refs.refRecruitApplyDetailModal.seq_recruit = this.seq
      this.$refs.refRecruitApplyDetailModal.show()
    },
    refresher (done) {
    ...
    selectListMax() {
      ...
      this.$axios.get('/api/recruit/selectRecruitApplyListLastPageNum',
        {params: {uid: this.getUid, pageSize: this.pageSize, keyword: this.keyword, seq_recruit: this.seq}})
        .then((result) => {
    ...
    selectList(idx, done) {
      ...
      this.$axios.get('/api/recruit/selectRecruitApplyList',
        {params: {uid: this.getUid, pageNum: idx, pageSize: this.pageSize, keyword: this.keyword, seq_recruit: this.seq}})
        .then((result) => {
          // console.log(JSON.stringify(result.data))
