<template>
  <q-page class="page-default">
    ...
    <div class="row q-pl-md row justify-center q-pt-sm q-pb-sm">
      <div class="col-12">
        {{ $t('menu_pop_token_list_description') }}
      </div>
    </div>
...
export default defineComponent({
  ...
  methods: {
    ...
    goDetail(seq) {
      // 상세 화면으로 이동
      this.$router.push({ path: '/token/detail', query: { s: seq }})
    ...
    // 인기 토큰 리스트 마지막 페이지 조회
    selectListMax() {
      // 검색어 입력창 x버튼 클릭시 this.keyword가 null이 됨.
      if (!this.keyword) {
        this.keyword = ''
      }
      this.$axios.get('/api/token/selectPopTokenListLastPageNum',
    ...
    selectList(idx, done) {
      ...
      this.$axios.get('/api/token/selectPopTokenList',
