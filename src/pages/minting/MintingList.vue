<template>
  <q-page class="page-default">
    ...
    <div class="row q-pl-md row justify-center q-pt-sm q-pb-sm">
      <div class="col-12">
        {{ $t('menu_minting_list_description') }}
      </div>
    </div>
    ...
    <q-pull-to-refresh @refresh="refresher">
      <q-infinite-scroll @load="loadMore" :offset="0" ref="infiniteScroll">
        ...
        <div v-for="item in mintingList" :key="item.seq" @click="goDetail(item.seq)" style="cursor: pointer; z-index: 1;">
          <q-separator />
          <q-item clickable>
            ...
            <q-item-section>
              <div class="row">
                ...
                <q-item-label class="col-3 text-left"></q-item-label>
                <q-item-label class="col-9 text-right">{{ $t('start') }} : {{ item.start_time }}</q-item-label>
                <q-item-label class="col-3 text-left"></q-item-label>
                <q-item-label class="col-9 text-right">{{ $t('end') }} : {{ item.end_time }}</q-item-label>
              </div>
            </q-item-section>
          </q-item>
        </div>
...
export default defineComponent({
  ...
  methods: {
    ...
    selectListMax() {
      ...
      this.$axios.get('/api/minting/selectMintingListLastPageNum',
    ...
    selectList(idx, done) {
      ...
      this.$axios.get('/api/minting/selectMintingList',
        {params: {uid: this.getUid, pageNum: idx, pageSize: this.pageSize, keyword: this.keyword}})
        .then((result) => {
          // console.log(JSON.stringify(result.data))
          // console.log(result.data)
          if (idx === 1) { // 첫번째 load인 경우
            this.mintingList = [] // 리스트 초기화
          }
          this.mintingList = this.mintingList.concat(result.data)

          // nft_image가 video 타입이면 isVideoNft를 true로 설정
          for (let i = 0; i < this.mintingList.length; i++) {
            const nft_image = this.mintingList[i].nft_image
            ...
            if (
              ...
              this.mintingList[i].isVideoNft = true
            } else {
              this.mintingList[i].isVideoNft = false
            }
          }

          // 데이터 없음 표시 설정
          if (!this.mintingList || this.mintingList.length < 1) {
