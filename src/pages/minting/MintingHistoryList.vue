  ...
  <q-page class="page-default">
    <div class="row q-pt-md q-pl-md justify-center">
      <div class="col-12 doc-heading doc-h2">
        {{ $t('menu_minting_history_list') }}
      </div>
    </div>
    <q-separator />
    <div class="row q-pl-md row justify-center q-pt-sm q-pb-sm">
      <div class="col-12">
        {{ $t('menu_minting_history_list_description') }}
      </div>
    </div>
    ...
    <div class="q-pt-sm q-pb-md q-pl-md row">
      ...
      <!-- &nbsp;&nbsp;
      <q-btn @click="goRegisterMintingHistory" icon="add" color="primary" size="lg" style="width: 80px;" /> -->
    </div>
    ...
    <q-pull-to-refresh @refresh="refresher">
      <q-infinite-scroll @load="loadMore" :offset="0" ref="infiniteScroll">
        ...
        <div v-for="item in mintingHistoryList" :key="item.seq">
          <q-separator />
          <q-item>
            <q-item-section avatar>
              <q-avatar>
                <img v-if="item.pay_token_logo_image" :src="item.pay_token_logo_image">
            ...
            <q-item-section>
              <div class="row">
                <q-item-label class="col-12 text-weight-bold">
                  <span class="">{{ item.pay_token_amount }}</span>
                  &nbsp;<span class="">{{ item.pay_token_symbol }}</span>
                  &nbsp;<span class="">(Amount: {{ Number(item.amount) }})</span>
                </q-item-label>
                <q-item-label class="col-12">{{ item.wallet_address }} - {{ item.wallet_type }}</q-item-label>
                <q-item-label v-if="item.option_field_value" class="col-12">Options: {{ item.option_field_value }}</q-item-label>
                <q-item-label class="col-12"><a @click="openUrl('https://scope.klaytn.com/tx/' + item.txid_minting)" style="cursor: pointer;z-index: 1;"><b>{{ item.txid_minting }}</b></a></q-item-label>
              </div>
            </q-item-section>
          </q-item>
...
export default defineComponent({
  ...
  methods: {
    ...
    // NFT 민팅 거래내역 마지막 페이지 조회
    selectListMax() {
      // 검색어 입력창 x버튼 클릭시 this.keyword가 null이 됨.
      if (!this.keyword) {
        this.keyword = ''
      }
      this.$axios.get('/api/minting/selectMintingHistoryListLastPageNum',
        {params: {uid: this.getUid, seq_minting: this.seq, pageSize: this.pageSize, keyword: this.keyword}})
        .then((result) => {
    ...
    selectList(idx, done) {
      ...
      this.$axios.get('/api/minting/selectMintingHistoryList',
        {params: {uid: this.getUid, seq_minting: this.seq, pageNum: idx, pageSize: this.pageSize, keyword: this.keyword}})
        .then((result) => {
          // console.log(JSON.stringify(result.data))
          if (idx === 1) { // 첫번째 load인 경우
            this.mintingHistoryList = [] // 리스트 초기화
          }
          this.mintingHistoryList = this.mintingHistoryList.concat(result.data)

          // 데이터 없음 표시 설정
          if (!this.mintingHistoryList || this.mintingHistoryList.length < 1) {
