      ...
      <q-infinite-scroll @load="loadMore" :offset="0" ref="infiniteScroll">
        ...
        <q-card class="my-card q-mb-lg" v-for="item in tokenList" :key="item.seq">
          ...
          <div class="text-h6" style="word-break: break-all;">
            ...
            SEQ: {{ item.seq }} | {{ item.name }} ({{ item.symbol }})
          </div>
          ...
          <!-- <q-img src="images/token_temp_image.png">
            ...
            <div class="absolute-bottom">
              ...
              <div class="text-subtitle2" style="word-break: break-all;">{{ $t('token_initial_supply') }} : {{ Number(item.initial_supply).toLocaleString() }}</div>
...
export default defineComponent({
  ...
  methods: {
    ...
    async goPayment(item) {
      this.$router.push({ path: '/admin/adminCreate', query: { key: item.seq }}) // 토큰 생성 화면으로 이동
    },
