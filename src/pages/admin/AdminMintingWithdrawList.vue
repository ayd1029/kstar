<template>
  <q-page class="page-default">
    <div class="row q-pt-md q-pl-md justify-center">
      <div class="col-12 doc-heading doc-h2">
        [A] {{ $t('menu_admin_minting_withdraw_list') }}
      </div>
    </div>
    ...
    <div class="row q-pl-md row justify-center q-pt-sm q-pb-sm">
      <div class="col-12">
        {{ $t('menu_admin_minting_withdraw_list_description') }}
      </div>
    </div>
    ...
    <q-pull-to-refresh @refresh="refresher">
      <q-infinite-scroll @load="loadMore" :offset="0" ref="infiniteScroll">
        ...
        <div v-for="item in withdrawList" :key="item.seq" @click="showDetail(item.seq)" style="cursor: pointer;z-index: 1;">
          <q-separator />
          <q-item clickable>
            <q-item-section avatar class="q-pt-xs">
              <div v-if="item.nft_image">
                <video v-if="item.isVideoNft" :src="item.nft_image" autoplay loop muted style="width: 100%; max-width: 100px;"></video>
                <img v-else :src="item.nft_image" style="width: 100%; max-width: 100px" />
              </div>
              <div v-else>
            ...
            <q-item-section>
              <div class="row">
                ...
                <q-item-label class="col-12">SEQ : {{ item.seq}}</q-item-label>
                <q-item-label class="col-12 text-red">{{ $t('minting_withdraw_time') }} : {{ qDate.formatDate(item.withdraw_time, 'YYYY-MM-DD HH:mm:ss') }}</q-item-label>
              </div>
            </q-item-section>
          </q-item>
        </div>
  ...
  <MintingWithdrawModal ref="refMintingWithdrawModal"/>
</template>
...
export default defineComponent({
  ...
  methods: {
    ...
    showDetail(seq) {
      ...
      this.$refs.refMintingWithdrawModal.seq = seq
      this.$refs.refMintingWithdrawModal.show()
    },
    refresher (done) {
    ...
    // NFT 민팅 자금 요청 리스트 마지막 페이지 조회
    selectListMax() {
      // 검색어 입력창 x버튼 클릭시 this.keyword가 null이 됨.
      if (!this.keyword) {
        this.keyword = ''
      }
      this.$axios.get('/api/admin/selectAdminMintingWithdrawListLastPageNum',
      ...
      this.$axios.get('/api/admin/selectAdminMintingWithdrawList',
        ...
        .then((result) => {
          ...
          for (let i = 0; i < this.withdrawList.length; i++) {
            const nft_image = this.withdrawList[i].nft_image
            ...
            if (
              ...
              this.withdrawList[i].isVideoNft = true
            } else {
              this.withdrawList[i].isVideoNft = false
            }
          }

          // 데이터 없음 표시 설정
          if (!this.withdrawList || this.withdrawList.length < 1) {
