<template>
  <q-page class="page-default">
    <div class="row q-pt-md q-pl-md justify-center">
      <div class="col-12 doc-heading doc-h2">
        [A] {{ $t('menu_admin_presale_withdraw_list') }}
      </div>
    </div>
    ...
    <div class="row q-pl-md row justify-center q-pt-sm q-pb-sm">
      <div class="col-12">
        {{ $t('menu_admin_presale_withdraw_list_description') }}
      </div>
    </div>
  ...
  <PresaleWithdrawModal ref="refPresaleWithdrawModal"/>
</template>
...
export default defineComponent({
  ...
  methods: {
    ...
    showDetail(seq) {
      ...
      this.$refs.refPresaleWithdrawModal.seq = seq
      this.$refs.refPresaleWithdrawModal.show()
    },
    refresher (done) {
    ...
    // 사전판매 자금 요청 리스트 마지막 페이지 조회
    selectListMax() {
      // 검색어 입력창 x버튼 클릭시 this.keyword가 null이 됨.
      if (!this.keyword) {
        this.keyword = ''
      }
      this.$axios.get('/api/admin/selectAdminPresaleWithdrawListLastPageNum',
    ...
    selectList(idx, done) {
      ...
      this.$axios.get('/api/admin/selectAdminPresaleWithdrawList',
