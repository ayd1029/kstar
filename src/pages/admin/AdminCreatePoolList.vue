  ...
  <q-page class="page-default">
    <div class="row q-pt-md q-pl-md justify-center">
      <div class="col-12 doc-heading doc-h2">
        [A] {{ $t('menu_admin_create_pool_request_list') }}
      </div>
    </div>
    ...
    <div class="row q-pl-md row justify-center q-pt-sm q-pb-sm">
      <div class="col-12">
        {{ $t('menu_admin_create_pool_request_list_description') }}
      </div>
    </div>
    ...
    <q-pull-to-refresh @refresh="refresher">
      <q-infinite-scroll @load="loadMore" :offset="0" ref="infiniteScroll">
        ...
        <div v-for="item in poolList" :key="item.seq" @click="showDetail(item.seq)" style="cursor: pointer;z-index: 1;">
          <q-separator />
          <q-item clickable>
            <q-item-section avatar>
              <q-avatar>
                <img v-if="item.target_token_logo_image" :src="item.target_token_logo_image">
            ...
            <q-item-section>
              <div class="row">
                <q-item-label class="col-12 text-weight-bold">{{ item.target_token_symbol }} + {{ item.pair_token_symbol }}</q-item-label>
...
export default defineComponent({
  ...
  methods: {
    ...
    showDetail(seq) {
      ...
      this.$refs.refPoolModal.seq = seq
      this.$refs.refPoolModal.show()
    },
    refresher (done) {
    ...
    // 유동성 풀 리스트 마지막 페이지 조회
    selectListMax() {
      // 검색어 입력창 x버튼 클릭시 this.keyword가 null이 됨.
      if (!this.keyword) {
        this.keyword = ''
      }
      this.$axios.get('/api/admin/selectAdminPoolListLastPageNum',
    ...
    selectList(idx, done) {
      ...
      this.$axios.get('/api/admin/selectAdminPoolList',
