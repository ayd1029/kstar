<template>
  <q-page class="page-default">
    <div class="row q-pt-md q-pl-md justify-center">
      <div class="col-12 doc-heading doc-h2">
        [A] {{ $t('menu_admin_contract_verification_request_list') }}
      </div>
    </div>
    ...
    <div class="row q-pl-md row justify-center q-pt-sm q-pb-sm">
      <div class="col-12">
        {{ $t('menu_admin_contract_verification_request_list_description') }}
      </div>
    </div>
    ...
    <q-pull-to-refresh @refresh="refresher">
      <q-infinite-scroll @load="loadMore" :offset="0" ref="infiniteScroll">
        ...
        <div v-for="item in tokenContractVerifytList" :key="item.seq" @click="showDetail(item.seq)" style="cursor: pointer;z-index: 1;">
          <q-separator />
          <q-item clickable>
            ...
            <q-item-section>
              <div class="row">
                <q-item-label class="col-12 text-weight-bold">{{ item.token_name }} <span class="text-grey-7">{{ item.symbol }}</span></q-item-label>
                <q-item-label class="col-12">{{ item.status_name }}</q-item-label>
                <q-item-label class="col-12">updated : {{ qDate.formatDate(item.mod_time, 'YYYY-MM-DD HH:mm:ss') }}</q-item-label>
              </div>
            </q-item-section>
          </q-item>
        </div>
  ...
  <TokenContractVerifyModal ref="refTokenContractVerifyModal"/>
</template>
...
export default defineComponent({
  name: 'AdminContractVerifyList',
  data () {
    return {
      refresherDone: '',
      pageSize: 20,
      lastPageNum: 1, // 마지막 페이지
      tokenContractVerifytList: [],
      noDataFlag: false,
      keyword: '', // 검색키워드
    }
  },
  ...
  methods: {
    ...
    showDetail(seq) {
      ...
      this.$refs.refTokenContractVerifyModal.seq = seq
      this.$refs.refTokenContractVerifyModal.show()
    },
    refresher (done) {
    ...
    selectListMax() {
      ...
      this.$axios.get('/api/admin/selectAdminTokenContractVerifyListLastPageNum',
    ...
    selectList(idx, done) {
      ...
      this.$axios.get('/api/admin/selectAdminTokenContractVerifyList',
        {params: {uid: this.getUid, pageNum: idx, pageSize: this.pageSize, keyword: this.keyword}})
        .then((result) => {
          // console.log(JSON.stringify(result.data))
          // console.log(result.data)
          if (idx === 1) { // 첫번째 load인 경우
            this.tokenContractVerifytList = [] // 리스트 초기화
          }
          this.tokenContractVerifytList = this.tokenContractVerifytList.concat(result.data)

          // 데이터 없음 표시 설정
          if (!this.tokenContractVerifytList || this.tokenContractVerifytList.length < 1) {
