  ...
  <q-page class="q-pa-md page-default">
    <div class="row justify-center">
      <div class="col-12 doc-heading doc-h2">
        {{ $t('menu_presale_register') }}
      </div>
    </div>
    <q-separator />
    <div class="row justify-center q-pt-sm q-pb-sm">
      <div class="col-12">
        {{ $t('menu_presale_register_description') }}
      </div>
    </div>
    ...
    <div class="row justify-center q-pt-lg">
      ...
      <div class="col-6 text-right">
        <q-btn class="btn" color="secondary" text-color="black" size="lg" style="width: 98%;" @click="registerPresale" tabindex="36">
          <b>{{ $t('register') }}</b>
        </q-btn>
      </div>
    </div>
...
export default defineComponent({
  ...
  created: function () {
    ...
    // 사용자가 등록한 토큰 리스트 조회
    this.selectMyTokenList()

    // 사용자가 등록한 화이트리스트 리스트 조회
    this.selectMyWhitelistList()
  ...
  methods: {
    ...
    async selectMyTokenList() {
      const param = {
        uid: this.getUid,
      }
      const result = await this.$axios.get('/api/presale/selectMyTokenList', { params: { ...param } })
      if (result.data) {
        // console.log(result.data)
        this.targetTokenList = result.data
        this.targetToken = this.targetTokenList[0]
        this.token_contract_address = this.targetTokenList[0].contract_address
        this.token_name = this.targetTokenList[0].name
        this.token_symbol = this.targetTokenList[0].symbol
        this.token_decimals = this.targetTokenList[0].decimals
        // this.token_total_supply = this.targetTokenList[0].initial_supply
        this.token_total_supply = this.targetTokenList[0].initial_supply.substring(0, this.targetTokenList[0].initial_supply.length - this.targetTokenList[0].decimals)
        this.token_logo_image = this.targetTokenList[0].logo_image
        this.official_website = this.targetTokenList[0].official_website
        this.official_email = this.targetTokenList[0].official_email
        this.whitepaper = this.targetTokenList[0].docs
        this.targetTokenOptions = ref(this.targetTokenList) // 검색용 리스트 설정
      } else {
    ...
    async selectMyWhitelistList() {
      const param = {
        uid: this.getUid,
      }
      const result = await this.$axios.get('/api/whitelist/selectMyWhitelistComboList', { params: { ...param } })
      if (result.data && result.data.length > 0) {
        // console.log(result.data)
        this.targetWhitelistList = result.data
        this.targetWhitelist = this.targetWhitelistList[0]
        this.whitelist_seq = this.targetWhitelistList[0].seq
        this.targetWhitelistOptions = ref(this.targetWhitelistList) // 검색용 리스트 설정
      } else {
        this.targetWhitelistList = []
        this.targetWhitelist = []
        this.whitelist_seq = null
        this.targetWhitelistOptions = [] // 검색용 리스트 설정
      }
    },
    ...
    registerPresale() {
      ...
      const param = {
        ...
        type: this.type,
        price: this.type === 'FIXED' ? this.price : null,
      ...
      this.$axios.post('/api/presale/insertPresale', param)
        .then((result) => {
          // console.log(JSON.stringify(result.data))
